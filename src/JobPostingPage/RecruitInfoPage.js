import React from 'react'
import './RecruitInfoPage.css'
import NavBar from "../MainPage/MainComponent/NavBar.js";

const RecruitInfoPage = () => {
  return (
    <div className="element">
      <div className="frame">
        <div className="div">
          <div className="frame-2">
            <div className="text-wrapper">채용 공고</div>
            <div className="frame-3">
              <div className="frame-4">
                <div className="frame-5">
                  <div className="frame-6">
                    <div className="text-wrapper-2">(주)엔미디어플랫폼</div>
                    <img
                      className="img"
                      alt="Favorite"
                      src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66ba069ad632e20f0c1152a0/img/favorite@2x.png"
                    />
                  </div>
                  <div className="frame-7">
                    <p className="p">Nexon Company window application 개발 엔지니어 모집</p>
                    <img
                      className="grade"
                      alt="Grade"
                      src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66ba069ad632e20f0c1152a0/img/grade@2x.png"
                    />
                  </div>
                </div>
                <img
                  className="line"
                  alt="Line"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66ba069ad632e20f0c1152a0/img/line-2.png"
                />
                <div className="frame-5">
                  <div className="frame-8">
                    <div className="frame-9">
                      <div className="text-wrapper-3">3년 경력</div>
                      <img
                        className="img"
                        alt="Arrow upward alt"
                        src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66ba069ad632e20f0c1152a0/img/arrow-upward-alt@2x.png"
                      />
                    </div>
                    <div className="frame-9">
                      <div className="text-wrapper-3">학력 무관</div>
                    </div>
                    <div className="frame-9">
                      <div className="text-wrapper-3">서울 강남구</div>
                    </div>
                    <div className="text-wrapper-3">정규직</div>
                  </div>
                  <p className="text-wrapper-4">등록 10/3(월) ~ 마감 11/20(수)</p>
                </div>
              </div>
              <div className="frame-10">
                <div className="text-wrapper-5">지원하기</div>
              </div>
            </div>
          </div>
          <div className="rectangle" />
          <div className="frame-11">
            <img
              className="image"
              alt="Image"
              src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2d8cf4d8f7eb28bb7ce11/img/image-2.png"
            />
            <div className="frame-5">
              <div className="text-wrapper-6">기업정보</div>
              <div className="frame-12">
                <div className="frame-5">
                  <div className="text-wrapper-7">(주)그레이고</div>
                  <p className="text-wrapper-8">서울시 마포구 동교로 104 (서교동) 4층</p>
                </div>
                <div className="frame-13">
                  <div className="frame-14">
                    <div className="frame-15">
                      <div className="text-wrapper-9">산업</div>
                      <div className="text-wrapper-10">설립</div>
                    </div>
                    <div className="frame-15">
                      <div className="frame-16">
                        <div className="text-wrapper-11">광고 대행업</div>
                      </div>
                      <div className="frame-16">
                        <div className="text-wrapper-11">2016년 (9년차)</div>
                      </div>
                    </div>
                  </div>
                  <div className="frame-14">
                    <div className="frame-15">
                      <div className="text-wrapper-9">사원수</div>
                      <div className="text-wrapper-10">기업형태</div>
                    </div>
                    <div className="frame-15">
                      <div className="frame-16">
                        <div className="text-wrapper-11">5명</div>
                      </div>
                      <div className="frame-16">
                        <div className="text-wrapper-11">중소기업 (비상장)</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <img
            className="line-2"
            alt="Line"
            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2d8cf4d8f7eb28bb7ce11/img/line-17.png"
          />
          <div className="frame-17">
            <p className="div-2">
              <span className="span">(주)그레이고</span>
              <span className="text-wrapper-12"> 경력자가 남긴 댓글을 확인해보세요!</span>
            </p>
            <div className="frame-18">
              <div className="frame-19">
                <div className="frame-20">
                  <div className="frame-21">
                    <img
                      className="img"
                      alt="Account circle"
                      src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2c247830accd7d866283e/img/account-circle@2x.png"
                    />
                    <div className="text-wrapper-13">wldsaam5234</div>
                  </div>
                  <div className="text-wrapper-14">5일 전</div>
                </div>
                <div className="text-wrapper-15">ㅇㅇ 이거임</div>
              </div>
              <div className="frame-19">
                <div className="frame-20">
                  <div className="frame-21">
                    <img
                      className="img"
                      alt="Account circle"
                      src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2c247830accd7d866283e/img/account-circle@2x.png"
                    />
                    <div className="text-wrapper-13">wldsaam5234</div>
                  </div>
                  <div className="text-wrapper-14">5일 전</div>
                </div>
                <div className="text-wrapper-15">ㅇㅇ 이거임</div>
              </div>
              <div className="frame-19">
                <div className="frame-20">
                  <div className="frame-21">
                    <img
                      className="img"
                      alt="Account circle"
                      src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2c247830accd7d866283e/img/account-circle@2x.png"
                    />
                    <div className="text-wrapper-13">wldsaam5234</div>
                  </div>
                  <div className="text-wrapper-14">5일 전</div>
                </div>
                <div className="text-wrapper-15">ㅇㅇ 이거임</div>
              </div>
              <div className="frame-19">
                <div className="frame-20">
                  <div className="frame-21">
                    <img
                      className="img"
                      alt="Account circle"
                      src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2c247830accd7d866283e/img/account-circle@2x.png"
                    />
                    <div className="text-wrapper-13">wldsaam5234</div>
                  </div>
                  <div className="text-wrapper-14">5일 전</div>
                </div>
                <div className="text-wrapper-15">ㅇㅇ 이거임</div>
              </div>
            </div>
          </div>
        </div>
        <div className="frame-22">
          <img
            className="format-list-bulleted"
            alt="Format list bulleted"
            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2d8cf4d8f7eb28bb7ce11/img/format-list-bulleted@2x.png"
          />
          <div className="text-wrapper-16">목록 보기</div>
        </div>
      </div>
      <NavBar authenticate={authenticate} setAuthenticate={setAuthenticate} userName={userName} />
    </div>
  );
}

export default RecruitInfoPage