import React from 'react'
import './MyScrap.css'

const MyScrap = () => {
  return (
    <div className="MyScrap">
      <div className="MyScrap-text-wrapper-2">스크랩한 공고</div>
      <div className="MyScrap-frame-wrapper">
        <div className="MyScrap-frame-5">
          <div className="MyScrap-frame-6">
            <div className="MyScrap-text-wrapper-3">전체 (3건)</div>
          </div>
          <div className="MyScrap-frame-7">
            <div className="MyScrap-frame-8">
              <div className="MyScrap-frame-9">
                <img
                  className="MyScrap-img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank@2x.png"
                />
                <div className="MyScrap-text-wrapper-4">선택한 공고 삭제</div>
                <div className="MyScrap-frame-10">
                  <div className="MyScrap-text-wrapper">최신순</div>
                  <img
                    className="MyScrap-img"
                    alt="Keyboard arrow down"
                    src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/keyboard-arrow-down@2x.png"
                  />
                </div>
              </div>
              <img
                className="MyScrap-line"
                alt="Line"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-2.png"
              />
            </div>
            <div className="MyScrap-frame-11">
              <div className="MyScrap-frame-12">
                <img
                  className="MyScrap-img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank-1@2x.png"
                />
                <div className="MyScrap-frame-13">
                  <div className="MyScrap-frame-14">
                    <div className="MyScrap-frame-15">
                      <div className="MyScrap-text-wrapper-5">(주)엔미디어플랫폼</div>
                      <img
                        className="MyScrap-img"
                        alt="Favorite"
                        src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/favorite@2x.png"
                      />
                    </div>
                    <p className="MyScrap-p">등록 10/3(월) ~ 마감 11/20(수)</p>
                  </div>
                  <div className="MyScrap-frame-16">
                    <div className="MyScrap-frame-17">
                      <div className="MyScrap-frame-18">
                        <p className="MyScrap-text-wrapper-6">Nexon Company window application 개발 엔지니어 모집</p>
                        <img
                          className="MyScrap-grade"
                          alt="Grade"
                          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/grade@2x.png"
                        />
                      </div>
                      <div className="MyScrap-frame-19">
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">3년 경력</div>
                          <img
                            className="MyScrap-img"
                            alt="Arrow upward alt"
                            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/arrow-upward-alt@2x.png"
                          />
                        </div>
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">학력 무관</div>
                        </div>
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">서울 강남구</div>
                        </div>
                        <div className="MyScrap-text-wrapper-7">정규직</div>
                      </div>
                    </div>
                    <div className="MyScrap-frame-21">
                      <div className="MyScrap-text-wrapper-8">지원하기</div>
                    </div>
                  </div>
                </div>
              </div>
              <img
                className="MyScrap-line"
                alt="Line"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-2.png"
              />
              <div className="MyScrap-frame-12">
                <img
                  className="MyScrap-img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank-1@2x.png"
                />
                <div className="MyScrap-frame-13">
                  <div className="MyScrap-frame-14">
                    <div className="MyScrap-frame-15">
                      <div className="MyScrap-text-wrapper-5">(주)엔미디어플랫폼</div>
                      <img
                        className="MyScrap-img"
                        alt="Favorite"
                        src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/favorite@2x.png"
                      />
                    </div>
                    <p className="MyScrap-p">등록 10/3(월) ~ 마감 10/25(금)</p>
                  </div>
                  <div className="MyScrap-frame-16">
                    <div className="MyScrap-frame-17">
                      <div className="MyScrap-frame-18">
                        <p className="MyScrap-text-wrapper-6">Nexon Company window application 개발 엔지니어 모집</p>
                        <img
                          className="MyScrap-grade"
                          alt="Grade"
                          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/grade@2x.png"
                        />
                      </div>
                      <div className="MyScrap-frame-19">
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">3년 경력</div>
                          <img
                            className="MyScrap-img"
                            alt="Arrow upward alt"
                            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/arrow-upward-alt@2x.png"
                          />
                        </div>
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">학력 무관</div>
                        </div>
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">서울 강남구</div>
                        </div>
                        <div className="MyScrap-text-wrapper-7">정규직</div>
                      </div>
                    </div>
                    <div className="MyScrap-frame-22">
                      <div className="MyScrap-text-wrapper-8">마감된 공고</div>
                    </div>
                  </div>
                </div>
              </div>
              <img
                className="MyScrap-line"
                alt="Line"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-2.png"
              />
              <div className="MyScrap-frame-12">
                <img
                  className="MyScrap-img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank-1@2x.png"
                />
                <div className="MyScrap-frame-13">
                  <div className="MyScrap-frame-14">
                    <div className="MyScrap-frame-15">
                      <div className="MyScrap-text-wrapper-5">(주)엔미디어플랫폼</div>
                      <img
                        className="MyScrap-img"
                        alt="Favorite"
                        src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/favorite@2x.png"
                      />
                    </div>
                    <p className="MyScrap-p">등록 10/3(월) ~ 마감 10/25(금)</p>
                  </div>
                  <div className="MyScrap-frame-16">
                    <div className="MyScrap-frame-17">
                      <div className="MyScrap-frame-18">
                        <p className="MyScrap-text-wrapper-6">Nexon Company window application 개발 엔지니어 모집</p>
                        <img
                          className="MyScrap-grade"
                          alt="Grade"
                          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/grade@2x.png"
                        />
                      </div>
                      <div className="frame-19">
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">3년 경력</div>
                          <img
                            className="MyScrap-img"
                            alt="Arrow upward alt"
                            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/arrow-upward-alt@2x.png"
                          />
                        </div>
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">학력 무관</div>
                        </div>
                        <div className="MyScrap-frame-20">
                          <div className="MyScrap-text-wrapper-7">서울 강남구</div>
                        </div>
                        <div className="MyScrap-text-wrapper-7">정규직</div>
                      </div>
                    </div>
                    <div className="MyScrap-frame-22">
                      <div className="MyScrap-text-wrapper-8">마감된 공고</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default MyScrap